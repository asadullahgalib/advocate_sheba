  ...
  <div class="col-md-12" style="padding-top: 40px;">
    <div class="card">
      <div class="card-header">
        <h3 class="card-title">Slider List
          <a href="{{ route('human-resource.hrm.slider.add') }}" class="btn btn-success float-right btn-sm custom_btn"><i class="fa fa-plus-circle"> Add Slider</i></a>
        </h3>
      </div>
        ...
        <table id="example1" class="table table-bordered">
          ...
          @foreach($allData as $key=> $value)
            <tr class="text-center">
              <td>{{ $key + 1 }}</td>
              <td>
                <img src="{{(!empty(@$value->image))?url('uploads/slider_images/'.@$value->image):url('uploads/no_image.png')}}" width="50%">
              </td>
              <td>
