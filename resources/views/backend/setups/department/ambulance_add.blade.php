      ...
      <div class="card-header">
        <h3 class="card-title">
          ...
          @else
            Ambulance Add
          @endif
          <a href="{{ route('human-resource.hrm.ambulance.view') }}" class="btn btn-success float-right btn-sm custom_btn"><i class="fa fa-list"> Ambulance List</i></a>
        </h3>
      </div>

      <form role="form" action="{{(@$editData)?route('human-resource.hrm.ambulance.update',$editData->id):route('human-resource.hrm.ambulance.store')}}" method="POST" enctype="multipart/form-data" id="MyForm">
        @csrf
          <div class="card-body">
            <div class="form-row">                   
              ...
              <div class="form-group col-sm-2" style="z-index: 100;">
                <img id="showImage" src="{{(!empty($editData->image))?url('public/upload/ambulance_images/'.$editData->image):url('public/upload/no_image.png')}}" style="width: 150px; height: 150px" class="form-control">
              </div>
