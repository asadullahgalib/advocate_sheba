      ...
      <div class="row">
        <div class="col-md-12">
          <div class="card">
            ...
            <div class="card-body">
              <form role="form" action="{{ route('setups.assign.subject.update',$assignSubjects[0]->class_id) }}" method="POST" id="MyForm">
              @csrf
                <div class="add_item"> 
                  <div class="form-row"> 
                      ...
                      <select name="class_id" class="form-control select2bs4">
                        <option value="">Select Class</option>
                          @foreach($classes as $class)
                            <option value="{{ $class->id }}" {{($assignSubjects[0]->class_id == $class->id)?"selected":""}}>{{ $class->name }}</option>
                          @endforeach
                      </select>                 
                    </div>                 
                  </div>
                          ...
                          <select name="subject_id[]" class="form-control">
                            <option value="">Select Subject</option>
                              @foreach($subjects as $subject)
                                <option value="{{ $subject->id }}" {{($assignSubject->subject_id == $subject->id)?"selected":""}}>{{ $subject->name }}</option>
                              @endforeach
                          </select>                 
                        </div>
                        <div class="form-group col-md-2">
                          ...
                          <select name="group_id[]" class="form-control">
                            <option value="">Select Group</option>
                              @foreach($groups as $group)
                                <option value="{{ $group->id }}" {{($assignSubject->group_id == $group->id)?"selected":""}}>{{ $group->name }}</option>
                              @endforeach
                          </select>                 
                        </div>
                        <div class="form-group col-md-2">
                          ...
                          <select name="subject_status[]" class="form-control">
                            ...
                            <option value="1" {{($assignSubject->subject_status == '1')?"selected":""}}>Main</option>
                            <option value="2" {{($assignSubject->subject_status == '2')?"selected":""}}>Optional</option>
                            <option value="3" {{($assignSubject->subject_status == '3')?"selected":""}}>Extra</option>
                          </select>                 
                        </div>
