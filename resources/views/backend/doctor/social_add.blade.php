      ...
      <div class="card-header">
        <h3 class="card-title">
          ...
          @else
            Social Media Add
          @endif
          <a href="{{ route('doctor-profile.social.view') }}" class="btn btn-success float-right btn-sm custom_btn"><i class="fa fa-list"> Social Media List</i></a>
        </h3>
      </div>

      <form role="form" action="{{(@$editData)?route('doctor-profile.social.update',$editData->id):route('doctor-profile.social.store')}}" method="POST" enctype="multipart/form-data" id="MyForm">
        @csrf
          <div class="card-body">
            <div class="form-row">
              ...
              <div class="form-group col-md-4">
                <label for="name">Profile Name <span style="color:red">*</span> </label>
                <input type="text" name="name" class="form-control form-control-sm" value="{{ @$editData->name }}"> 
              </div>
              <div class="form-group col-md-2">
                <label for="name">Icon <span style="color:red">*</span></label>
                <input type="text" name="icon" class="form-control form-control-sm" value="{{ @$editData->icon }}">
              </div>
              ...
              <div class="form-group col-sm-3" style="z-index: 100;">
                <img id="showImage" src="{{(!empty($editData->image))?url('public/upload/social_images/'.$editData->image):url('public/upload/no_image.png')}}" style="width: 150px; height: 150px" class="form-control">
              </div>
