...
@section('content')
  ...
  <div class="col-md-12" style="padding-top: 40px;">
    <div class="card">
      ...
      <div class="card-body">
				<form method="POST" action="{{route('payroll.booking.report.pdf')}}" id="myForm" target="_blank">
					@csrf
					<div class="form-row">
						<div class="col-md-5">
              <label>Doctor Info</label>
              <select name="doctor_id" id="doctor_id" class="form-control form-control-sm select2">
              	<option value="">Select Doctor</option>
              	@foreach($doctors as $doctor)
              	<option value="{{$doctor->doctor_id}}">{{@$doctor['doctor']['name']}} - {{@$doctor['doctor']['department']['name']}}</option>
              	@endforeach
              </select>
          	</div>
						<div class="col-md-2">
...
<script type="text/javascript">
	$(document).on('click','#search',function(){
		...
		var end_date = $('#end_date').val();
		$.ajax({
			url: "{{route('payroll.booking.report.handlebar')}}",
			type: "get",
			data: {
				'doctor_id': doctor_id,
